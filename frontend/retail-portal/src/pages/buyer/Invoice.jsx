              ...
              {hasRated ? (
                <div className="space-y-2">
                  <div className="flex gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`w-6 h-6 ${i < existingRating?.score ? 'fill-yellow-400 text-yellow-400' : 'text-slate-300'}`} />
                    ))}
                  </div>
                  {existingRating?.review && (
                    <p className="text-slate-600 italic bg-white p-3 rounded-lg border border-slate-100 shadow-sm mt-3">"{existingRating.review}"</p>
                  )}
                  <p className="text-xs text-emerald-600 font-bold uppercase tracking-wider mt-2">✓ Review Submitted</p>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star className={`w-8 h-8 ${(hoverRating || rating) >= star ? 'fill-yellow-400 text-yellow-400' : 'text-slate-300'}`} />
                      </button>
                    ))}
                  </div>
                  <textarea
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-bold rounded-xl transition shadow-sm"
                  >
